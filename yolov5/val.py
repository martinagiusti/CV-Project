...
def get_bbox_from_filename(filename):
...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[1]:
            ...
            total_inference_time += dt[1].t  # accumula il tempo di inferenza del batch
        ...
        for si, pred in enumerate(preds):
            ...
            if nl:
                ...
                correct = process_batch(predn, labelsn, iouv)
                    # Calcolo IoU medio per immagine
                ious = box_iou(labelsn[:, 1:], predn[:, :4])  # IoU matrix: GT x predictions
                mean_iou = ious.max(1).values.mean().item() if ious.numel() else 0.0
                iou_values.append((Path(paths[si]).name, mean_iou))

                if plots:
                    confusion_matrix.process_batch(predn, labelsn)
            else:
                    iou_values.append((paths[si].name, 0.0))
    ...
    # Calcolo statistiche IoU
    iou_threshold = 0.7
    num_above_threshold = sum(iou > iou_threshold for _, iou in iou_values)
    percent_above_threshold = 100.0 * num_above_threshold / len(iou_values)
    mean_inference_time = total_inference_time / seen
    # Print results
    ...
    LOGGER.info(f"Mean inference time per image: {mean_inference_time:.3f} s")
    LOGGER.info(f"Percentage of images with IoU > {iou_threshold}: {percent_above_threshold:.2f}%")

    # Salvo IoU su file
    iou_file = save_dir / 'iou_per_image.txt'
    with open(iou_file, 'w') as f:
        for filename, iou in iou_values:
            f.write(f'{filename}: {iou:.4f}\n')
    LOGGER.info(f"IoU per immagine salvate in {iou_file}")
    
    mean_iou_dataset = np.mean([iou for _, iou in iou_values])
    LOGGER.info(f"Mean IoU on the test dataset: {mean_iou_dataset:.4f}")
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    example_indices = random.sample(range(len(iou_values)), min(10, len(iou_values)))

    for idx in example_indices:
        filename, mean_iou = iou_values[idx]
        image_path = Path(data[task]) / filename
        label_path = Path(data[task]).as_posix().replace('/images/', '/labels/') + f'/{Path(filename).stem}.txt'
        label_path = Path(label_path)


    # carica immagine originale
        img = cv2.imread(str(image_path))
        assert img is not None, f"Non riesco a caricare {image_path}"
        h, w = img.shape[:2]

    # carica ground truth bbox
        labels = np.loadtxt(label_path).reshape(-1, 5)
        for cls, x_c, y_c, bw, bh in labels:
            x1_gt = int((x_c - bw / 2) * w)
            y1_gt = int((y_c - bh / 2) * h)
            x2_gt = int((x_c + bw / 2) * w)
            y2_gt = int((y_c + bh / 2) * h)
            cv2.rectangle(img, (x1_gt, y1_gt), (x2_gt, y2_gt), (0, 255, 0), 3)

    # predizione
        im_tensor = torch.from_numpy(img[..., ::-1].copy()).permute(2, 0, 1).unsqueeze(0).float().to(device) / 255
        pred = model(im_tensor)[0]
        pred = non_max_suppression(pred, conf_thres, iou_thres)[0]

        if pred is not None and len(pred):
            pred = pred.cpu().numpy()
            best_pred = pred[np.argmax(pred[:, 4])]
            x1, y1, x2, y2, conf, cls = best_pred
            x1, y1, x2, y2 = [int(x1), int(y1), int(x2), int(y2)]
            cv2.rectangle(img, (x1, y1), (x2, y2), (255, 0, 0), 3)
            cv2.putText(img, f"{conf:.2f}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0), 2)
            iou = calculate_iou([x1, y1, x2, y2], [x1_gt, y1_gt, x2_gt, y2_gt])
        else:
            iou = 0.0

        cv2.putText(img, f"IoU: {iou:.2f}", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)

        out_path = save_dir / f"example_{Path(filename).stem}.jpg"
        cv2.imwrite(str(out_path), img)

    LOGGER.info("10 esempi salvati nella cartella della run.")

    return (mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist(),
        mean_inference_time, percent_above_threshold, mean_iou_dataset), maps, t
